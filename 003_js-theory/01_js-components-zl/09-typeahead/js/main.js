...
input.addEventListener('input', ({ target }) => {
  ...
  const matches = countries.filter((country) => country.name.toLowerCase().startsWith(inputValue))
  const listItems = matches.map((item) => `<li>${boldSearchTerms(item.name, inputValue)}</li>`).join('')
  ul.innerHTML = listItems
  ul.removeAttribute('hidden')
})

document.addEventListener('click', ({ target }) => {
  if (!target.closest('.typeahed')) {
